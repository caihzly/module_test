...
extern enum techpoint_support_reso g_ch_reso[PAD_MAX];
...
static int check_chip_id(struct techpoint *techpoint)
{
	...
	if (chip_id_h == TP9930_CHIP_ID_H_VALUE &&
		...
		return 0;
		   ...
		   chip_id_l == TP2855_CHIP_ID_L_VALUE_2) {
		dev_info(&client->dev,
			 "techpoint check chip id CHIP_TP2855 !\n");
		techpoint->chip_id = CHIP_TP2855;
		techpoint->input_type = TECHPOINT_MIPI;
...
//TP9930 不支持混接
			...
			if (TECHPOINT_S_RESO_NO_SUPPORT == reso)
			{
				reso = g_ch_reso[ch];
			}
			else if (g_ch_reso[ch] != reso)
			{
				dev_err(&client->dev, "line : %d, ch = %d, pre_reso = %d, cur_reso = %d\n", __LINE__, ch, g_ch_reso[ch], reso);
				g_ch_reso[ch] = reso;
				tp2855_set_channel_reso(client, ch, reso);
			}
		}
...
#if 1 //Add by caihz
extern int tp9930_audio_init(struct techpoint *techpoint);
extern int tp2855_audio_init(struct techpoint *techpoint);
#endif
...
int techpoint_start_video_stream(struct techpoint *techpoint)
{
    ...
    if (techpoint->chip_id == CHIP_TP9930) {
		...
		tp9930_audio_init(techpoint);
    }
	else if (techpoint->chip_id == CHIP_TP2855) //Add by caihz
	{
		tp2855_audio_init(techpoint);
	}
	else
	{}
	up(&reg_sem);
...
int do_reset_rx_video_stream(struct techpoint *techpoint)
{
	int ret = 0;
	struct i2c_client *client = techpoint->client;
	...
	if (ret) 
	{
		dev_err(&client->dev, "techpoint_start_video_stream common_reg_list failed");
		return ret;
	}

...
enum techpoint_support_std g_default_std = STD_HDA;
enum techpoint_support_reso g_ch_reso[PAD_MAX] = {TECHPOINT_S_RESO_1080P_25, TECHPOINT_S_RESO_1080P_25, TECHPOINT_S_RESO_1080P_25, TECHPOINT_S_RESO_1080P_25};
			    ...
			    enum techpoint_support_reso reso)
{
	...
	case TECHPOINT_S_RESO_1080P_30:
		...
		techpoint_write_reg(client, 0x2a, 0x30); //正常模式
	...
	case TECHPOINT_S_RESO_NO_SUPPORT:
		dev_err(&client->dev, "set channel %d SD or No Support !!!\n", ch);
		techpoint_read_reg(client, 0xf5, &tmp);
		tmp &= ~SYS_MODE[ch];
		techpoint_write_reg(client, 0xf5, tmp);

		techpoint_write_reg(client, 0x2a, 0x38); //强制出黑屏
...
int tp2855_get_channel_reso(struct i2c_client *client, int ch)
{
	...
	case TP2855_CVSTD_SD:
		...
		break;
	default:
		//dev_err(&client->dev, "detect channel %d UNSUPPORT\n", ch);
		return TECHPOINT_S_RESO_NO_SUPPORT;
		break;
	}
...
{
	printk("=========== tp2855_set_quick_stream \n");

	if (stream) {
		printk("=========== tp2855_set_quick_stream on\n");
		techpoint_write_reg(client, 0x40, 0x8);
		techpoint_write_reg(client, 0x23, 0x0);
	} else {
		printk("=========== tp2855_set_quick_stream off\n");
		techpoint_write_reg(client, 0x40, 0x8);
...
int tp2855_video_init(struct i2c_client *client)
{
	techpoint_write_reg(client, 0x40, 0x04);

    techpoint_write_reg(client, 0x10, 0x10);
    techpoint_write_reg(client, 0x11, 0x45);
    techpoint_write_reg(client, 0x12, 0x35);  
